...
Use install.packages(“datapasta”) in console, then use addins-paste as
data frame.
...
![](wk10class19_files/figure-commonmark/unnamed-chunk-2-1.png)

Add some major milestone timepoints to our plot.

``` r
ggplot(cdc) + aes(x=year, y=cases) + geom_point() + geom_line() + geom_vline(xintercept=1947, col="darkgreen") + geom_vline(xintercept=1996, col = "red") + geom_vline(xintercept=2020, col="blue")
```

![](wk10class19_files/figure-commonmark/unnamed-chunk-3-1.png)

The full introduction of the wP (whole-cell) Pertussis vaccine
immunization in the mid 1940s lead to a dramatic reduction in case
numbers (from over 200,000 to 100s).

The switch to the aP (newer acellular formalization) doesn’t contain all
components that the old vaccine has. Also, there’s anti-vaccine
sentiments.

The 2020 lock-downs and social distancing measures increase the case
number again. COVID infection caused related diseases to happen.
...
They make their data available via JSON format API endpoints (database
tables in a key:value type format like “infancy_vac”:“wP”). To read
this, we can use the `read_json()` function from the **jsonlite**
package. install.packages(“jsonlite”)
...
We need to join or link these tables with the `subject` table so we can
begin to analyze this data and know who give Ab sample was collected for
and when.
...
Now let’s join the `ab_titer` table with are `meta` table so we have all
...
![](wk10class19_files/figure-commonmark/unnamed-chunk-18-1.png)
...
![](wk10class19_files/figure-commonmark/unnamed-chunk-20-1.png)

``` r
ggplot(igg) + aes(MFI_normalised, antigen, col=infancy_vac) + geom_boxplot()
```

![](wk10class19_files/figure-commonmark/unnamed-chunk-21-1.png)
...
![](wk10class19_files/figure-commonmark/unnamed-chunk-22-1.png)
...
![](wk10class19_files/figure-commonmark/unnamed-chunk-23-1.png)
...
![](wk10class19_files/figure-commonmark/unnamed-chunk-24-1.png)

This plot shows time course of Pertussis toxin (PT) antibody responses
for a large set of wP (teal color) and aP (red color) individuals.
Levels peak at day 14 and are larger in magnitude for wP than aP
individuals.
