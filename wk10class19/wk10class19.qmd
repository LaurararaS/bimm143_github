...
They make their data available via JSON format API endpoints (database tables in a key:value type format like "infancy_vac":"wP"). To read this, we can use the `read_json()` function from the **jsonlite** package. install.packages("jsonlite")
...
This breakdown is not particularly representative of the US population - this is a serious cavet for this study. However, it is still the largest sample of it's type every assembled.
...
Now let's join the `ab_titer` table with are `meta` table so we have all the information about a given Ab measurement.
